...
def unretweet_tweet(tweet_id):
    current_user_id = get_jwt_identity()
    retweet = Retweet.query.filter_by(
        user_id=current_user_id, tweet_id=tweet_id
    ).first()
    if retweet is None:
...
def get_user_tweets(user_id):
    tweets = (
        Tweet.query.filter_by(user_id=user_id).order_by(Tweet.timestamp.desc()).all()
    )
    return jsonify([tweet.to_dict() for tweet in tweets]), 200
...
def get_user_replies(user_id):
    replies = (
        Reply.query.filter_by(user_id=user_id).order_by(Reply.timestamp.desc()).all()
    )
